...
!***********************************************************************
      ...
      DIMENSION WINDMON(12),HEATMON(12) ! separately define the monthly wind speed and heat when adjust the parameter
...
!
      DO 55 K=1,KB
       IF(ZZ(K)*H.GT.-10.) THEN
        SB(K)=100.-100.*abs(ZZ(K)*H)/10.
        S(K)=SB(K)
        SF(K)=S(K)
       ENDIF
  55  CONTINUE
